 ...
 * @since 03.03.2019
 */
public class ArrStoreSpec extends InstructionSpec {

    public ArrStoreSpec( String mnemonic, int typeId, int dataLen ) {
    ...
    public void process( int level, Instruction instruction, Stack<Operand> stack, SourceCodeWriter writer, Context context ) {
        writer.writeln( level, stack.pop().getValue() + "[" + stack.pop().getValue() + "] = " + stack.pop().getValue() );
    }
}
